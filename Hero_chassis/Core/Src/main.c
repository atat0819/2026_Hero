...
int main(void)
{
  ...
  MX_UART8_Init();
  /* USER CODE BEGIN 2 */
 // 队列创建移到各自的任务中，避免在 FreeRTOS 启动前创建
...
motorspeedtargetQueue = xQueueCreate(10,sizeof(MotorSpeedTarget_t)); // 创建一个队列来传递电机目标转速数据

	motorCurrentDataQueue = xQueueCreate(1,sizeof(MotorCurrentData_t) * 4); // 创建一个队列来传递电机当前数据
    
    chassisCurrentDataQueue = xQueueCreate(10,sizeof(chassisCurrentData_t)); // 创建一个队列来传递底盘当前数据


//HAL::CAN::get_can_bus_instance(); // 触发 CAN bus 初始化：HAL_CAN_Start + 激活中断通知

// ❌ 删除：DMA 启动移到 remote_task 中，避免重复启动
