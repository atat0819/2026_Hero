...
uint8_t send_str2[sizeof(float) * 12U]; // 11 个 float 数据 + 帧尾 (4字节)
...
// 开 vofa 软件的 justfloat 模式：11 个 float 数据 + 帧尾 00 00 80 7F
// 打包用 memcpy（无对齐/别名风险），发送走 UART6 DMA（非阻塞，不拖累 5ms 控制环）
void vofa_send(float x1, float x2, float x3, float x4,
               float x5, float x6, float x7, float x8, float x9,
               float x10, float x11)
{
    const float values[11] = {x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11};
    const uint32_t frameTail = 0x7F800000U;

    memcpy(send_str2, values, sizeof(values));
    memcpy(&send_str2[sizeof(values)], &frameTail, sizeof(frameTail));
