...
private:
    ...
    uint8_t manual_reverse_pending = 0; // 反转请求: 边缘检测命中后置1, STOP 消费后清零
    ...
    static constexpr float SINGLE_SHOT_PHASE_CORRECTION_LIMIT = 8.0f;
    // ROLLBACK_MARKER_SINGLE_SHOT_PHASE_CORRECTION_END

    // 连发模式恒定转速 (RPM 对应电机输出轴)
    static constexpr float FORWARD_SPEED = (26.0f);

    // ---- 超时保护 (防止堵转卡死) ----
    static constexpr uint32_t SINGLE_SHOT_TIMEOUT_COUNT   = 220; // 单发超时 (ticks)
    ...
    // 视觉持续开火时, 每发弹丸之间等待此时间
    static constexpr uint32_t SINGLE_SHOT_COOLDOWN_TICKS = 1000;//视觉用
    ...
    static constexpr uint32_t SINGLE_SHOT_REMOTE_HOLD_TICKS = 20; // 遥控器单发最短抱角刹车 150ms
    static constexpr uint32_t SINGLE_SHOT_REMOTE_HOLD_TIMEOUT_TICKS = 30; // 遥控器抱角最长 300ms
    static constexpr float SINGLE_SHOT_SETTLE_THRESHOLD = 3.0f; // 输出轴角误差小于此值才认为锁稳
    static constexpr float SINGLE_SHOT_SETTLE_SPEED_RPM = 3.0f; // 转速低于此值才允许退出抱角
