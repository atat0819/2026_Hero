...
private:
    ...
    static constexpr float TARGET_OPENING_PERCENT = 80.0f;

    // 击发拨杆位置 (DT7: UP=1, MIDDLE=3, DOWN=2) — 边缘触发, 拨一下打一发
    static constexpr uint8_t FIRE_TRIGGER_POSITION = 1U;  // Remote::UP

    // 推杆伸出到位等待 (60 ticks = 300ms) — 标定: 观察推杆顶死时间
    static constexpr uint32_t PRIME_HOLD_TICKS = 60U;
    // 放气保持时间 (20 ticks = 100ms) — 弹丸出膛即可, 太长浪费气
    static constexpr uint32_t FIRE_HOLD_TICKS = 20U;
    // 充压恢复等待 (100 ticks = 500ms) — 决定连发节奏上限, 标定
    static constexpr uint32_t BACK_HOLD_TICKS = 100U;
